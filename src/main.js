...
// --- Cloud Files (Windows CFAPI) bridge (rebuilt minimal lifecycle) ---
...
function appendVfsDebugLog(message, extra = null) {
  const logPath = getVfsDebugLogPath();
  vfsProviderState.debugLogPath = logPath;

  const stamp = new Date().toISOString();
  let line = `${stamp} ${String(message || '').trim()}`;
...
function getVfsStatus() {
  return {
    ...
    provider: {
      ...vfsProviderState,
      recentLogs: vfsProviderRecentLogs.slice(-40),
    },
  };
}
...
function getVfsProviderConfigKey(config) {
  return JSON.stringify({
    syncRoot: String(config?.virtualFilesFolder || ''),
    remoteFolderId: String(config?.remoteFolderId || ''),
    workspaceId: String(config?.workspaceId ?? 0),
    pipeName: getVfsPipeName(),
  });
}
...
async function handleVfsRequest(req) {
  ...
  let client;
  try {
    const authResult = await getAuthenticatedClient(vfsActiveConfig);
    client = authResult.client;
  } catch (err) {
    return { id, ok: false, error: `Authentication failed: ${err?.message || err}` };
  }
...
async function startVfsServer() {
  ...
  appendVfsDebugLog('[server] starting', { pipeName, pipePath });

  vfsServer = net.createServer(socket => {
    appendVfsDebugLog('[server] client connected');
    ...
    const decode = createFrameDecoder(async msg => {
      ...
      if (!response) return;
      try {
        socket.write(frameEncode(response));
      } catch {
        // Best-effort only.
      }
    });
    ...
    socket.on('close', () => {
      vfsSockets.delete(socket);
      appendVfsDebugLog('[server] client closed');
    });
    socket.on('error', err => {
      vfsSockets.delete(socket);
      appendVfsDebugLog('[server] client error', { message: String(err?.message || err) });
    });
  });

  await new Promise((resolve, reject) => {
    vfsServer.once('error', reject);
    vfsServer.listen(pipePath, () => {
      appendVfsDebugLog('[server] listening', { pipeName, pipePath });
      resolve();
    });
  });
}
...
async function stopVfsServer() {
  if (!vfsServer) return;
  appendVfsDebugLog('[server] stopping');
  ...
  vfsServer = null;
  appendVfsDebugLog('[server] stopped');
}

async function stopVfsProvider() {
  vfsProviderStopRequested = true;
  vfsProviderStarting = false;
  vfsProviderConfigKey = '';

  const proc = vfsProviderProcess;
  vfsProviderProcess = null;

  vfsProviderState.starting = false;
  vfsProviderState.running = false;
  vfsProviderState.pid = null;
  vfsProviderState.lastEventAt = Date.now();
  publishVfsStatus();

  if (!proc) {
    appendVfsDebugLog('[provider] stop requested but no process was running');
    return;
  }

  appendVfsDebugLog('[provider] stop requested', { pid: proc.pid });
  ...
  await new Promise(resolve => {
    ...
    try {
      ...
      appendVfsDebugLog('[provider] kill failed', { message: String(err?.message || err) });
      finish();
      return;
    }

    setTimeout(() => {
      appendVfsDebugLog('[provider] stop wait timeout elapsed', { pid: proc.pid });
      finish();
    }, 5000);
  });
}
...
function resolveVfsProviderLaunch() {
  const envPath = String(process.env.BOLTBYTES_VFS_PROVIDER_PATH || '').trim();
  if (envPath) {
    return {
      ok: true,
      strategy: 'env-exe',
  ...
  if (!app.isPackaged) {
    ...
    return {
      ok: false,
      strategy: 'missing-dev-exe',
      cmd: '',
      args: [],
      cwd: providerDir,
      error: `Cloud Files provider exe was not found. Build it first: dotnet build "${path.join(providerDir, 'BoltBytesVfsProvider.csproj')}"`,
    };
  }
...
async function startVfsProvider(config) {
  ...
  vfsProviderState.launchArgs = providerArgs.slice();
  vfsProviderState.lastError = '';
  vfsProviderState.lastExitCode = null;
  vfsProviderState.lastExitSignal = null;
  vfsProviderState.lastEventAt = Date.now();
  vfsProviderState.debugLogPath = getVfsDebugLogPath();
  publishVfsStatus();

  if (!launch.ok) {
    vfsProviderState.running = false;
    vfsProviderState.starting = false;
    vfsProviderState.lastError = launch.error || 'Cloud Files provider launch is not available.';
    vfsProviderState.lastEventAt = Date.now();
    addActivity({ level: 'warning', message: `[Cloud Files] ${vfsProviderState.lastError}` });
    appendVfsDebugLog('[provider] launch unavailable', { launch, providerArgs });
    publishVfsStatus();
    return;
  }

  appendVfsDebugLog('[provider] launching', {
  ...
  try {
    ...
    vfsProviderState.starting = true;
    publishVfsStatus();

    const child = spawn(launch.cmd, providerArgs, {
      cwd: launch.cwd,
      windowsHide: true,
      stdio: ['ignore', 'pipe', 'pipe'],
    });

    vfsProviderProcess = child;
    vfsProviderStarting = false;
    vfsProviderState.starting = false;
    vfsProviderState.running = true;
    vfsProviderState.pid = child.pid ?? null;
    vfsProviderState.lastEventAt = Date.now();
    publishVfsStatus();

    appendVfsDebugLog('[provider] spawned', {
      pid: child.pid ?? null,
      spawnfile: child.spawnfile || launch.cmd,
      spawnargs: child.spawnargs || providerArgs,
    });
    ...
    child.on('error', err => {
      ...
      appendVfsDebugLog('[provider] process error', { message: msg, pid: child.pid ?? null });
      addActivity({ level: 'warning', message: `[Cloud Files] Provider error: ${msg}` });

      if (vfsProviderProcess === child) vfsProviderProcess = null;
      vfsProviderStarting = false;
      vfsProviderState.starting = false;
      vfsProviderState.running = false;
      vfsProviderState.pid = null;
    ...
    child.on('close', (code, signal) => {
      appendVfsDebugLog('[provider] close', {
        pid: child.pid ?? null,
        code: code ?? null,
        signal: signal || null,
      });
    ...
    child.on('exit', (code, signal) => {
      ...
      appendVfsDebugLog('[provider] exit', {
        pid: child.pid ?? null,
        requested,
        code: code ?? null,
        signal: signal || null,
      ...
      addActivity({
        level: abnormalExit ? 'warning' : 'info',
        message: `[Cloud Files] Provider stopped (${requested ? 'requested' : `code ${codeLabel}${signalLabel ? `, signal ${signalLabel}` : ''}`})`,
      });
      ...
      vfsProviderState.running = false;
      vfsProviderState.pid = null;
      vfsProviderState.lastExitCode = code ?? null;
      vfsProviderState.lastExitSignal = signalLabel || null;
      vfsProviderState.lastEventAt = Date.now();

      if (abnormalExit) {
        vfsProviderState.lastError = vfsProviderState.lastLogLine || `Provider exited with code ${codeLabel}${signalLabel ? ` (${signalLabel})` : ''}`;
      }
    ...
    const msg = String(err?.message || err);
    appendVfsDebugLog('[provider] spawn threw', { message: msg });
    vfsProviderProcess = null;
    vfsProviderStarting = false;
    vfsProviderState.starting = false;
    vfsProviderState.running = false;
...
async function configureVfsInfrastructure(config) {
  ...
  if (!enabled) {
    appendVfsDebugLog('[configure] disabling Cloud Files');
  ...
  const nextConfigKey = getVfsProviderConfigKey(nextConfig);
  const sameConfig = vfsProviderProcess && vfsProviderConfigKey === nextConfigKey;

  appendVfsDebugLog('[configure] applying', {
    sameConfig,
    nextConfigKey,
    currentConfigKey: vfsProviderConfigKey,
    hasProcess: Boolean(vfsProviderProcess),
    starting: Boolean(vfsProviderStarting),
  });

  vfsActiveConfig = nextConfig;
  ...
  await stopVfsProvider();
  vfsProviderConfigKey = nextConfigKey;
  await startVfsProvider(nextConfig);
}
