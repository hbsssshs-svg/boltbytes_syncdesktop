...
$target = Join-Path $projectRoot 'src\main.js'
if (-not (Test-Path $target)) {
  throw "Could not find src\main.js under $projectRoot"
}

$backup = "$target.bak.7fbc7bfe"
...
function getVfsDebugLogPath() {
  try {
    return path.join(app.getPath('userData'), 'cloud-files-provider-debug.log');
  } catch {
    return path.join(process.cwd(), 'cloud-files-provider-debug.log');
  }
}

function appendVfsDebug(event, fields = {}) {
  try {
    const payload = {
    ...
    const logPath = getVfsDebugLogPath();
    fsSync.mkdirSync(path.dirname(logPath), { recursive: true });
    fsSync.appendFileSync(logPath, line, 'utf8');
    vfsProviderState.debugLogPath = logPath;
  } catch {
    // Best-effort only.
  }
}

function pushVfsDebug(level, event, fields = {}) {
  appendVfsDebug(event, fields);
  const details = JSON.stringify(fields);
  const line = details === '{}' ? `[debug] ${event}` : `[debug] ${event} ${details}`;
  pushVfsProviderLog(level, line);
  addActivity({ level, message: `[Cloud Files] ${line}` });
}
...
async function startVfsServer() {
  ...
  pushVfsDebug('info', 'vfs-server-listening', { pipeName, pipePath });
}

async function stopVfsServer() {
  if (!vfsServer) {
    pushVfsDebug('info', 'vfs-server-stop-skip-missing');
    return;
  }
  pushVfsDebug('info', 'vfs-server-stopping', { socketCount: vfsSockets.size });
  ...
  vfsServer = null;
  pushVfsDebug('info', 'vfs-server-stopped');
}
...
async function stopVfsProvider() {
  ...
  if (!proc) {
    pushVfsDebug('info', 'provider-stop-skip-missing');
    return;
  }

  pushVfsDebug('info', 'provider-stop-requested', {
    pid: proc.pid || null,
    spawnfile: proc.spawnfile || '',
    spawnargs: Array.isArray(proc.spawnargs) ? proc.spawnargs : [],
  });
  ...
  await new Promise(resolve => {
    ...
    const finish = source => {
      ...
      try { proc.removeListener('close', onClose); } catch {}
      pushVfsDebug('info', 'provider-stop-finished', {
        source,
        pid: proc.pid || null,
      });
      resolve();
    ...
    const onExit = (code, signal) => finish(`exit:${String(code)}:${String(signal || '')}`);
    const onClose = code => finish(`close:${String(code)}`);
    ...
    try {
      proc.kill();
      pushVfsDebug('info', 'provider-kill-sent', { pid: proc.pid || null });
    } catch (err) {
      pushVfsDebug('warning', 'provider-kill-error', {
        pid: proc.pid || null,
...
function resolveVfsProviderLaunch() {
  ...
  if (envPath) {
    return { kind: 'env-exe', cmd: envPath, args: [], providerDir: path.dirname(envPath) };
  }

  if (!app.isPackaged) {
    const providerDir = path.join(app.getAppPath(), 'native', 'windows', 'BoltBytesVfsProvider');
    const csproj = path.join(providerDir, 'BoltBytesVfsProvider.csproj');
    const devExe = path.join(
      providerDir,
      'bin',
      'Debug',
      'net8.0-windows10.0.19041.0',
      'BoltBytesVfsProvider.exe'
    );

    if (fsSync.existsSync(devExe)) {
      return { kind: 'dev-exe', cmd: devExe, args: [], providerDir, csproj, devExe, devExeExists: true };
    }

    return {
      kind: 'dotnet-no-build',
      cmd: 'dotnet',
      args: ['run', '--project', csproj, '--no-build'],
      providerDir,
      csproj,
      devExe,
      devExeExists: false,
  ...
  return { kind: 'packaged-exe', cmd: candidate, args: [], providerDir: path.dirname(candidate) };
}
...
async function configureVfsInfrastructure(config) {
  ...
  pushVfsDebug('info', 'configure-vfs-called', {
    enabled,
    hasVirtualFilesFolder: Boolean(config?.virtualFilesFolder),
    hasRemoteFolderId: Boolean(config?.remoteFolderId),
    workspaceId: Number(config?.workspaceId ?? 0),
  });
  ...
  if (!enabled) {
    ...
    await stopVfsServer();
    pushVfsDebug('info', 'configure-vfs-disabled');
    return;
  }
  ...
  if (!config?.virtualFilesFolder) {
    ...
    pushVfsDebug('warning', 'configure-vfs-missing-folder');
    return;
  }
  ...
  if (!config?.remoteFolderId) {
    ...
    pushVfsDebug('warning', 'configure-vfs-missing-remote-folder');
    return;
  }
  ...
  if (vfsProviderProcess || vfsProviderStarting) {
    pushVfsDebug('info', 'provider-start-skipped-already-active', {
      hasProcess: Boolean(vfsProviderProcess),
      pid: vfsProviderProcess?.pid || null,
      starting: Boolean(vfsProviderStarting),
    });
    return;
  }
  ...
  const isDotnetLaunch = launch.kind === 'dotnet-no-build' || String(launch.cmd || '').toLowerCase() === 'dotnet';
  const providerArgs = [
  ...
  const args = isDotnetLaunch
    ? [...(launch.args || []), '--', ...providerArgs]
    : [...(launch.args || []), ...providerArgs];
  ...
  pushVfsDebug('info', 'provider-launch-resolved', {
    kind: launch.kind || 'unknown',
    cmd: launch.cmd || '',
    args,
    providerDir: launch.providerDir || '',
    csproj: launch.csproj || '',
    devExe: launch.devExe || '',
    devExeExists: Boolean(launch.devExeExists),
    pipeName,
    syncRoot: config.virtualFilesFolder,
    remoteFolderId: String(config.remoteFolderId),
    workspaceId: String(config.workspaceId ?? 0),
  ...
  try {
    ...
    const spawnOptions = {
      cwd: launch.providerDir || undefined,
      windowsHide: true,
      stdio: ['ignore', 'pipe', 'pipe'],
    };

    vfsProviderProcess = require('node:child_process').spawn(launch.cmd, args, spawnOptions);

    pushVfsDebug('info', 'provider-spawned', {
      pid: vfsProviderProcess.pid || null,
      spawnfile: vfsProviderProcess.spawnfile || launch.cmd || '',
      spawnargs: Array.isArray(vfsProviderProcess.spawnargs) ? vfsProviderProcess.spawnargs : [],
    ...
    vfsProviderProcess.on('spawn', () => {
      vfsProviderStarting = false;
      pushVfsDebug('info', 'provider-spawn-event', {
        pid: vfsProviderProcess?.pid || null,
      });
    });
    ...
    vfsProviderProcess.stdout?.on('data', buf => {
      ...
      for (const line of lines) {
        ...
        appendVfsDebug('provider-stdout', { line });
      }
    });
    ...
    vfsProviderProcess.stderr?.on('data', buf => {
      ...
      for (const line of lines) {
        ...
        appendVfsDebug('provider-stderr', { line });
      }
    });
    ...
    vfsProviderProcess.on('error', err => {
      ...
      pushVfsDebug('warning', 'provider-error', {
        pid: vfsProviderProcess?.pid || null,
        code,
        error: friendly,
      });
    ...
    vfsProviderProcess.on('close', (code, signal) => {
      pushVfsDebug('info', 'provider-close', {
        pid: vfsProviderProcess?.pid || null,
        code: code === null || typeof code === 'undefined' ? null : code,
        signal: signal || null,
      });
    });
    ...
    vfsProviderProcess.on('exit', (code, signal) => {
      ...
      pushVfsDebug('info', 'provider-exit', {
        pid: vfsProviderProcess?.pid || null,
        code: code === null || typeof code === 'undefined' ? null : code,
        signal: signal || null,
        requested,
        runtimeMs,
        lastError: vfsProviderState.lastError || '',
        lastLogLine: vfsProviderState.lastLogLine || '',
      });
      ...
      const abnormalExit = !requested && (Boolean(signal) || (typeof code === 'number' && code !== 0));
      if (abnormalExit && vfsActiveConfig?.virtualFilesEnabled && vfsActiveConfig?.virtualFilesFolder && vfsActiveConfig?.remoteFolderId) {
        ...
        vfsProviderRestartTimer = setTimeout(() => {
          ...
          configureVfsInfrastructure(vfsActiveConfig).catch(err => {
            const restartError = String(err?.message || err);
            pushVfsDebug('warning', 'provider-auto-restart-failed', { error: restartError });
            addActivity({ level: 'warning', message: `[Cloud Files] Auto-restart failed: ${restartError}` });
          });
        }, delayMs);

        pushVfsDebug('info', 'provider-auto-restart-scheduled', {
          attempt: vfsProviderRestartAttempts,
          delayMs,
        });
        addActivity({ level: 'info', message: `[Cloud Files] Auto-restart scheduled in ${Math.round(delayMs / 1000)}s.` });
      } else {
        vfsProviderRestartAttempts = 0;
        pushVfsDebug('info', 'provider-auto-restart-skipped', {
          requested,
          code: code === null || typeof code === 'undefined' ? null : code,
          signal: signal || null,
        });
    ...
    vfsProviderStarting = false;
    const msg = String(err?.message || err);
    pushVfsDebug('warning', 'provider-start-failed', { error: msg });
...
Write-Host "Run: node --check src\main.js"
